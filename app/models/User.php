...
class User extends \HXPHP\System\Model{
	...
	public static function atualizar($user_id, array $post){
		...
		if(!is_null($exists_mail) && intval($user_id)!== intval($exists_mail->id)){
			array_push($callbackObj->errors,'Oops! Já existe um usuário com este e-mail cadastrado.<br/>Por favor, escolha outro e tente novamente');
		...
		if(!is_null($exists_username) && intval($user_id)!== intval($exists_username->id)){
			array_push($callbackObj->errors,'Oops! Já existe um usuário com o login <strong>'.$post['username'].'</strong> cadastrado.<br/>Por favor, escolha outro e tente novamente');
			$callbackObj->user = $user;
			return $callbackObj;
		}
